...
{ 
    ...
    { 
        ...
        min_idx = i; 
        for (j = i+1; j < n; j++) 
          if (processSize[j] < processSize[min_idx]) 
            min_idx = j; 
  
        // Swap the found minimum element with the first element 
        //swap(&processSize[min_idx], &processSize[i]); 
        temp = processSize[min_idx]; 
        processSize[min_idx] = processSize[i]; 
        processSize[i] = temp; 
    } 
